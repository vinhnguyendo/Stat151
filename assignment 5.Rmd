...
exp(fixef(Q1))/(1+exp(fixef(Q1)))

test<-fixef(Q1)
 
#Creating a function that calulates the predicted probabilies for the typical type of student 
calc_predProbs <- function(mod){
  
  logodds<-fixef(mod)
  
  for (i in 2:length(logodds)){
    
    logodds[i]<-sum(test[c(1:i)])
